...
#define EDITOR_WORKING
	...
	void SPPKGame::Tick(float dt)
	{
		...
		{	// 테스트 코드 ㅡ 개발용
			...
			//SPtr<CAsset<CActor>> test = CAssetManager::LoadAssetDefault<CActor>(Globals::GetGameAssetPath(wtext("/actor/monkey.asset")));
			//auto testActor = new CStaticMeshActor(this);
			//testActor->CopyFromTemplate(test->getContentClass());

			SPtr<CAsset<CMap>> testMapTemplate = CAssetManager::LoadAssetDefault<CMap>(Globals::GetGameAssetPath(wtext("/map/testMap.asset")));
		...
		// 여기부터 
			...
			// 액터 강제 로드 테스트		
			SPtr<CAsset<CActor>> loadingActorTemplate = CAssetManager::LoadAssetDefault<CActor>(actorAssetPath);
		...
		//SPtr<CAsset<CActor>> testActorTemplate = CAssetManager::LoadAssetDefault<CActor>(assetRoot + wtext("testActorTemplate.asset"));
		...
		SPtr<CAsset<CResourceModel>> amA = CAssetManager::LoadAssetDefault<CResourceModel>(assetRoot + wtext("/models/cube/cube.asset"));
		애셋모델 저장하기 */

	//	// 맵 로드하고 액터들 다시 복구하기
	//	SPtr<CAsset<CMap>> testMapTemplate = CAssetManager::LoadAssetDefault<CMap>(assetRoot + wtext("/map/testMapTemplate.asset"));
